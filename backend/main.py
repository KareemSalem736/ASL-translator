...
def predict(input_data: LandmarkSequenceInput):
    ...
    # Call both motion detection funcs
    #j_result = is_j_motion(landmarks)
    #z_result = is_z_motion(landmarks)

    # Debug prints
    #print(f"J changes: {j_result['changes']}, detected: {j_result['detected']}")
    #print(f"Z changes: {z_result['changes']}, detected: {z_result['detected']}")

    # Decide based on who passes thresholds and has bigger zig-zag sums
    #j_sum = np.sum(j_result['changes'])
    #z_sum = np.sum(z_result['changes'])

    #motion_detected = None
    #if j_result['detected'] and (j_sum > z_sum or not z_result['detected']):
    #    motion_detected = "J"
    #elif z_result['detected']:
    #    motion_detected = "Z"
