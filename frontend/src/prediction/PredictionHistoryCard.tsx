...
const PredictionHistoryCard = () => {
  ...
  return (
    <Card
      ...
      nullContentMessage={isAuthenticated ? "No predictions yet." : "Please sign in to see history results."}
      ...
      <div className="list-group list-group-flush flex-grow-1 overflow-y-auto">
        {history.map((result) => (
          ...
          >
            ...
            <span className="text-muted small flex-shrink-0" style={{ width: '70px' }}>
                <div>{formatDate(result.created_at).datePart}</div>
                <div>{formatDate(result.created_at).timePart}</div>
            </span>

            {/* Right side: Content */}
            <span className="text-end flex-grow-1 text-break text-center" style={{ minWidth: 0 }}>
                {result.content}
            </span>
          </div>
        ))}
